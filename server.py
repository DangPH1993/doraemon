# VERSION: v31.50 — review questions unified to 4-choice MCQ — welcome/review de-dup + applied grammar review + wrong-answer persistence
# VERSION: v31.48 — completion state + review schedule for vocabulary/grammar
SERVER_FREE_CHAT_TUTOR_VERSION = "free-chat-tutor-v4-evidence-vocab-grammar-focus-v31.50-review-mcq4"
...
def _review_grammar_question_is_applied(question):
    """Accept only applied grammar MCQs with a blank and four short options."""
    q=dict(question or {})
    if str(q.get('question_type') or '').strip()!='multiple_choice': return False
    options=q.get('options') if isinstance(q.get('options'),list) else []
    if len(options)!=4: return False
    question_text=str(q.get('question') or '').strip()
    if not question_text: return False
    # Grammar review is always a sentence-application task with a blank.
    if not re.search(r'_{3,}',question_text): return False
    theory_markers=(
        'ý nghĩa của cấu trúc','nghĩa của cấu trúc','ý nghĩa tiếng việt',
        'cấu trúc này có nghĩa','được dùng như thế nào','cách dùng của cấu trúc',
        'ngữ pháp nào','định nghĩa của cấu trúc','what does this structure mean',
        'what is the meaning of','how is this structure used','which grammar rule'
    )
    if any(x in question_text.casefold() for x in theory_markers): return False
    # Options must look like completions, not explanations/theory paragraphs.
    for x in options:
        t=re.sub(r'^[A-D][.)]\s*','',str(x or '').strip(),flags=re.I)
        if not t or len(t)>120: return False
        if any(marker in t.casefold() for marker in theory_markers): return False
    return True


def _review_grammar_fallback_question(item):
    """Build an applied grammar MCQ-4 from a DB-backed example when possible."""
    row=dict(item or {})
    ...
    def _verb_forms(base):
        base=base.strip()
        irregular={
            'begin':('began','begun'),'steal':('stole','stolen'),'go':('went','gone'),
            'eat':('ate','eaten'),'do':('did','done'),'make':('made','made'),
            'take':('took','taken'),'write':('wrote','written'),'see':('saw','seen'),
            'come':('came','come'),'run':('ran','run'),'have':('had','had'),
            'give':('gave','given'),'get':('got','gotten'),'choose':('chose','chosen'),
            'speak':('spoke','spoken'),'drive':('drove','driven'),'ride':('rode','ridden'),
            'sing':('sang','sung'),'swim':('swam','swum'),'teach':('taught','taught'),
        }
        if base.casefold() in irregular:
            past,p3=irregular[base.casefold()]
        elif base.endswith('e') and not base.endswith('ee'):
            past=base+'d'; p3=past
        else:
            past=base+'ed'; p3=past
        if base.endswith('ie'):
            ing=base[:-2]+'ying'
        elif base.endswith('e') and not base.endswith('ee'):
            ing=base[:-1]+'ing'
        elif len(base)>=3 and base[-1] not in 'aeiouyw' and base[-2] in 'aeiou' and base[-3] not in 'aeiou':
            ing=base+base[-1]+'ing'
        else:
            ing=base+'ing'
        return past,p3,ing
    ...
    if ('to v' in low or 'to + v' in low or 'infinitive' in low) and m:
        base=m.group(1); past,p3,ing=_verb_forms(base); answer='to '+base; distractors=[base,ing,past]
    else:
        m=re.search(r'\b([A-Za-z][A-Za-z\'-]{2,}ing)\b',example,re.I)
        if ('v-ing' in low or 'gerund' in low or 'verb-ing' in low) and m:
            ing=m.group(1); stem=ing[:-3]; past,p3,_=_verb_forms(stem); answer=ing; distractors=[stem,'to '+stem,past]
        else:
            m=re.search(r'\b([A-Za-z][A-Za-z\'-]{2,}(?:ed|en))\b',example,re.I)
            if ('v3' in low or 'past participle' in low or 'past-participle' in low) and m:
                form=m.group(1); stem=form[:-2] if form.endswith('ed') else form; past,p3,ing=_verb_forms(stem); answer=form; distractors=[stem,ing,past]
    if not answer: return None
    masked=re.sub(r'\b'+re.escape(answer)+r'\b','____',example,count=1,flags=re.I)
    if masked==example: return None
    vals=[answer]+distractors
    if len(vals)!=4 or len({x.casefold() for x in vals})!=4: return None
    random.shuffle(vals)
    option_letters={k:vals[i] for i,k in enumerate(letters)}
    correct=next(k for k,v in option_letters.items() if v.casefold()==answer.casefold())
    return {
        'item_type':'grammar','item_id':int(row.get('id') or 0),'question_type':'multiple_choice',
        'question':f'Hoàn thành câu sau bằng cách chọn đáp án đúng theo ngữ pháp {pattern or "của bài"}:\n🇬🇧 {masked}',
        'options':[f'{k}. {option_letters[k]}' for k in letters],
        'option_letters':option_letters,'answer':correct,
        'answer_text':option_letters[correct],'answer_criteria':pattern or answer,
        'pattern':pattern,'meaning':str(row.get('meaning') or '').strip(),
        'explanation':str(row.get('explanation') or '').strip(),'example':example,
    }

def _review_vocab_question(item, all_vocab, mode=None):
    """Create a vocabulary review question in the only supported format: MCQ-4.

    Distractors come from other DB vocabulary meanings, preferring the same lesson
    and then the same course. No generic category distractors are generated.
    """
    ...
    if not meaning:
        return None
    letters=('A','B','C','D')
    lesson=str(item.get('source_lesson') or item.get('lesson') or '').strip().casefold()
    same_lesson=[]; same_course=[]
    seen=set()
    for other in all_vocab or []:
        oid=int(other.get('id') or 0)
        if oid==item_id: continue
        m=str(other.get('meaning') or '').strip()
        if not m or m.casefold()==meaning.casefold(): continue
        key=m.casefold()
        if key in seen: continue
        seen.add(key)
        other_lesson=str(other.get('source_lesson') or other.get('lesson') or '').strip().casefold()
        (same_lesson if lesson and other_lesson==lesson else same_course).append(m)
    distractors=(same_lesson+same_course)[:3]
    if len(distractors)<3:
        return None
    options=[meaning]+distractors
    random.shuffle(options)
    option_letters={k:options[i] for i,k in enumerate(letters)}
    answer=next(k for k,v in option_letters.items() if v.casefold()==meaning.casefold())
    return {
        'item_type':'vocabulary','item_id':item_id,'question_type':'multiple_choice',
        'question':f'📘 **{shown}**\n\nTừ này có nghĩa tiếng Việt nào dưới đây?',
        'options':[f'{k}. {option_letters[k]}' for k in letters],
        'option_letters':option_letters,'answer':answer,'answer_text':meaning,
        'answer_criteria':meaning,
    }

def _review_genai_one_call(course_id, data, max_q, lesson=None, only_failed=False):
    """Build review questions in ONE format only: four-choice applied MCQ."""
    vocab=list(data.get('vocabulary') or [])
    grammar=list(data.get('grammar') or [])
    max_q=max(1,min(12,int(max_q or 12)))
    random.shuffle(vocab); random.shuffle(grammar)
    if only_failed:
        vocab=vocab[:max_q]; grammar=grammar[:max(0,max_q-len(vocab))]
    else:
        vocab=vocab[:9]; grammar=grammar[:3]
        if len(vocab)+len(grammar)>max_q:
            grammar=grammar[:max(0,max_q-len(vocab))]; vocab=vocab[:max_q]

    selected_ids={('vocabulary',int(x.get('id') or 0)) for x in vocab}
    selected_ids.update({('grammar',int(x.get('id') or 0)) for x in grammar})
    prompt_data={
        'task':'Tạo quiz ôn tập chỉ bằng trắc nghiệm 4 đáp án A/B/C/D.',
        'lesson':str(lesson or ''),
        'rules':[
            'Mỗi câu bắt buộc question_type=multiple_choice và đúng 4 lựa chọn.',
            'Chỉ dùng item_id có trong DATA.',
            'Vocabulary: hỏi trực tiếp từ đang ôn; 4 option phải là nghĩa/cách đọc của các từ trong DATA, ưu tiên cùng lesson. Không dùng đáp án chung chung.',
            'Grammar: bắt buộc là bài tập áp dụng. Question phải có chỗ trống ____; 4 option phải là 4 khả năng hoàn thành CÙNG vị trí ngữ pháp, xoay quanh đúng pattern/động từ/cấu trúc trong DATA.',
            'Tuyệt đối không hỏi định nghĩa, ý nghĩa lý thuyết, tên cấu trúc hoặc cách dùng chung chung.',
            'Trả JSON duy nhất dạng {"questions":[{"item_type":"vocabulary"|"grammar","item_id":number,"question_type":"multiple_choice","question":"...____...","options":["A. ...","B. ...","C. ...","D. ..."],"option_letters":{"A":"...","B":"...","C":"...","D":"..."},"answer":"A","answer_text":"..."}]}.',
        ],
        'DATA':{'vocabulary':vocab,'grammar':grammar},
    }
    try:
        reply,_,_=_generate_chat_reply(
            'Bạn là gia sư Doraemon. Chỉ tạo quiz MCQ-4 đúng dữ liệu DB.\n'+json.dumps(prompt_data,ensure_ascii=False,separators=(',',':')),
            content_type=None,request_id=f'review-{course_id}-{int(time.time()*1000)}',
        ...
        print(f'[REVIEW MCQ] generation failed: {type(exc).__name__}: {exc}')
        generated=[]

    by_key={}
    for q in generated:
        if not isinstance(q,dict): continue
        try: key=(str(q.get('item_type') or '').strip(),int(q.get('item_id') or 0))
        except Exception: continue
        if key in selected_ids and str(q.get('question_type') or '')=='multiple_choice' and len(q.get('options') or [])==4:
            by_key.setdefault(key,q)

    questions=[]
    for item in vocab:
        q=_review_vocab_question(item,vocab)
        if not q: continue
        ai=by_key.get(('vocabulary',int(item.get('id') or 0))) or {}
        om=ai.get('option_letters') if isinstance(ai.get('option_letters'),dict) else {}
        meaning=str(item.get('meaning') or '').strip()
        if len(om)==4 and meaning:
            vals=[str(om.get(k) or '').strip() for k in ('A','B','C','D')]
            if all(vals) and len({v.casefold() for v in vals})==4 and meaning.casefold() in {v.casefold() for v in vals}:
                q['option_letters']={k:vals[i] for i,k in enumerate(('A','B','C','D'))}
                q['options']=[f'{k}. {q["option_letters"][k]}' for k in ('A','B','C','D')]
                q['answer']=next(k for k,v in q['option_letters'].items() if v.casefold()==meaning.casefold())
        questions.append(q)

    for item in grammar:
        iid=int(item.get('id') or 0)
        ai=by_key.get(('grammar',iid)) or {}
        q=None
        if ai and _review_grammar_question_is_applied(ai):
            om=ai.get('option_letters') if isinstance(ai.get('option_letters'),dict) else {}
            vals=[str(om.get(k) or '').strip() for k in ('A','B','C','D')]
            ans=str(ai.get('answer') or '').strip().upper()
            if all(vals) and len({v.casefold() for v in vals})==4 and ans in {'A','B','C','D'}:
                q={
                    'item_type':'grammar','item_id':iid,'question_type':'multiple_choice',
                    'question':str(ai.get('question') or '').strip(),
                    'options':[f'{k}. {vals[i]}' for i,k in enumerate(('A','B','C','D'))],
                    'option_letters':{k:vals[i] for i,k in enumerate(('A','B','C','D'))},
                    'answer':ans,'answer_text':vals['ABCD'.index(ans)],
                    'answer_criteria':str(item.get('pattern') or item.get('meaning') or '').strip(),
        ...
        if q is None:
            ...
            if q: print(f'[REVIEW GRAMMAR FALLBACK] item_id={iid} lesson={lesson!r} mode=applied_mcq4')
        if q: questions.append(q)
        else: print(f'[REVIEW GRAMMAR VALIDATION] skipped_no_mcq4 item_id={iid} lesson={lesson!r}')

    seen=set(); out=[]
    for q in questions:
        key=(str(q.get('item_type') or ''),int(q.get('item_id') or 0))
        if key in seen: continue
        if str(q.get('question_type') or '')!='multiple_choice' or len(q.get('options') or [])!=4: continue
        seen.add(key); out.append(q)
        if len(out)>=max_q: break
    print(f'[REVIEW MCQ STATE] course_id={course_id} lesson={lesson!r} questions={len(out)} format=MCQ4')
    return out
...
def _review_question_blocks(session_id, question, index, total):
    """Render the only supported review format: one four-option MCQ."""
    q=dict(question or {})
...
def _review_source_lesson_questions(course_id, lesson, content_type, max_questions=8):
    """Create four-choice applied MCQs from the exact published curriculum source."""
    ...
    for r in rows:
        ...
        if not text and isinstance(content.get('text'),str): text=str(content.get('text') or '').strip()
        if text: source_parts.append(f'[{code}]\n{text[:18000]}')
    source='\n\n'.join(source_parts).strip()
    if not source: return []
    n=max(1,min(8,int(max_questions or 8)))
    prompt=f"""Bạn là Doraemon, tạo quiz ôn tập cho bài {lesson} đã publish trong DB.
LOẠI: {content_type}

BẮT BUỘC:
- Chỉ tạo trắc nghiệm 4 đáp án A/B/C/D.
- Tối đa {n} câu.
- Ngữ pháp: phải là bài tập áp dụng vào câu, question có ____; 4 option là 4 khả năng hoàn thành cùng vị trí, xoay quanh chính grammar pattern/động từ đang ôn.
- Không hỏi định nghĩa, ý nghĩa, tên cấu trúc hoặc cách dùng chung chung.
- Từ vựng: 4 option phải là nghĩa/cách đọc/cách dùng liên quan trực tiếp đến các từ trong nguồn, ưu tiên các từ cùng bài.
- Chỉ dùng dữ liệu trong NGUỒN DB.
- Trả JSON duy nhất: {{"questions":[{{"question_type":"multiple_choice","question":"...____...","options":["A. ...","B. ...","C. ...","D. ..."],"option_letters":{{"A":"...","B":"...","C":"...","D":"..."}},"answer":"A","answer_text":"..."}}]}}

NGUỒN DB:
{source}
"""
    try:
        reply,_,_=_generate_chat_reply('Bạn là gia sư Doraemon. Chỉ tạo quiz MCQ-4 đúng nguồn DB.\n'+prompt,
            content_type=str(content_type or '') or None,request_id=f'review-source-{int(course_id)}-{int(time.time()*1000)}',
            gen_started=time.perf_counter(),user_text='',reasoning_profile='low')
        parsed=_review_json_from_text(reply); generated=(parsed.get('questions') if isinstance(parsed,dict) else []) or []
    except Exception as exc:
    ...
    for idx,q in enumerate(generated[:n]):
        if not isinstance(q,dict): continue
        if str(q.get('question_type') or '')!='multiple_choice': continue
        if str(content_type or '').strip()=='Ngữ pháp' and not _review_grammar_question_is_applied(q):
            print(f'[REVIEW SOURCE QUIZ] rejected non_applied_mcq4 lesson={lesson!r}'); continue
        om=q.get('option_letters') if isinstance(q.get('option_letters'),dict) else {}
        clean=[str(om.get(k) or '').strip() for k in ('A','B','C','D')]
        if not all(clean) or len(set(x.casefold() for x in clean))!=4:
            opts=q.get('options') if isinstance(q.get('options'),list) else []
            clean=[]
            for x in opts:
                val=re.sub(r'^[A-D][.)]\s*','',str(x or '').strip(),flags=re.I)
                if val and val.casefold() not in {y.casefold() for y in clean}: clean.append(val)
                if len(clean)>=4: break
        ans=str(q.get('answer') or '').strip().upper(); qtext=str(q.get('question') or '').strip()
        if len(clean)!=4 or ans not in {'A','B','C','D'} or not qtext: continue
        om={k:clean[i] for i,k in enumerate(('A','B','C','D'))}
        if str(content_type or '').strip()=='Ngữ pháp':
            proxy=_ensure_review_grammar_master(int(course_id),str(lesson).strip(),question_text=qtext,answer=ans,
                pattern=f'{lesson} · Ôn tập',meaning=str(q.get('answer_text') or om[ans]).strip(),
                explanation='Câu ôn được tạo từ nội dung bài đã publish trong curriculum_steps.')
            if not proxy: continue
            item_type='grammar'; item_id=int(proxy)
        else:
            item_type='lesson'; item_id=-(idx+1)
        questions.append({'item_type':item_type,'item_id':item_id,'question_type':'multiple_choice',
            'question':qtext,'options':[f'{k}. {om[k]}' for k in ('A','B','C','D')],
            'option_letters':om,'answer':ans,'answer_text':str(q.get('answer_text') or om[ans]).strip(),
            'answer_criteria':str(q.get('answer_text') or om[ans]).strip(),'source_lesson':str(lesson).strip()})
    print(f'[REVIEW SOURCE QUIZ] course_id={course_id} content_type={content_type!r} lesson={lesson!r} questions={len(questions)} format=MCQ4')
    return questions
...
def _start_review_chat_session(user_id, course_id, course_name, lesson=None, content_type=None, chatbox_id=None, max_questions=12, only_failed=False, all_failed_due=False):
    ...
    if not questions:
        ...
        for item in (data.get('grammar') or []):
            ...
            if q:
                questions.append(q)
    questions=[q for q in (questions or []) if str(q.get('question_type') or '')=='multiple_choice' and len(q.get('options') or [])==4]
    if not questions:
        raise HTTPException(500,'Không có câu hỏi MCQ-4 hợp lệ từ dữ liệu DB để tạo phiên ôn tập.')
    print(f'[REVIEW FORMAT GUARANTEE] user={user_id} course_id={course_id} lesson={source_lesson!r} questions={len(questions)} format=MCQ4')
    ...
    try:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            ...
            if finished:
                ...
                if review_scope == 'FULL' and source_lesson and source_lesson!='review_due':
                ...
                next_msg,next_blocks=_build_post_review_next_step(user_id,course_id,'',review_scope,source_lesson)
