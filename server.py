...
async def admin_knowledge_upload(
    ...
    # Đọc PDF từ UploadFile. Luôn chuyển dữ liệu thành một file-like object
    # có seek/tell/read để pypdf không bao giờ nhận bytes trực tiếp.
    raw=await file.read()
    ...
    try:
        ...
        # Materialize pages while stream is still alive.
        pdf_pages=list(reader.pages)
        if not pdf_pages:
            raise ValueError("PDF không có trang nào.")
        records_meta=normalize_kb_records(metadata_json,len(pdf_pages))
    except ValueError as e:
        raise HTTPException(400,str(e))
    except Exception as first_error:
        # Fallback: dùng file thật trên disk. Đây là phương án an toàn cho
        # một số PDF/pypdf kết hợp có lỗi khi parser làm việc với BytesIO.
        import tempfile
        try:
            ...
            raise HTTPException(400,f"Không đọc được PDF: {fallback_error}")
        finally:
            if temp_path:
                try:
                    os.unlink(temp_path)
    ...
    total=0
    try:
        for page_no,page in enumerate(pdf_pages,1):
    ...
    return {"success":True,"filename":source_file,"subject":subject,
            "pages":len(pdf_pages),"chunks":total,"records":len(records_meta),
