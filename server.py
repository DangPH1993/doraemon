...
SERVER_VERSION = "2026-08-12-upload-form-fix-v2"
...
async def admin_knowledge_upload(
    ...
    # UploadFile trả về bytes khi gọi read().
    # pypdf cần một file-like object có read/seek/tell, vì vậy tuyệt đối
    # không truyền bytes trực tiếp cho PdfReader. Dùng BytesIO và seek(0).
    raw=await file.read()
    if not raw:
    ...
    try:
        ...
        import traceback
        traceback.print_exc()
        raise HTTPException(400,f"Không đọc được PDF: {type(e).__name__}: {e}")
    ...
    try:
        ...
        raise HTTPException(500,f"Lỗi embedding/Pinecone: {type(e).__name__}: {e}")
    finally:
        try:
            pdf_stream.close()
        except Exception:
