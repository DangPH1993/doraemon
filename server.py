...
def _classify_chat_followup(text: str, history):
    ...
    The classifier receives only the latest user/model exchange plus the current
    ...
    if not raw:
        return False
    hist = list(history or [])[-2:]  # exactly 1 user/model exchange
    context = "\n".join(
...
def _classify_learning_intent(text: str):
    ...
    It receives only the current user sentence. Database state is intentionally
    excluded from the classifier; DB state is applied deterministically after the
    intent is known.
    """
    raw=str(text or '').strip()
    if not raw:
        return 'OTHER'
    prompt=(
        'Phân loại ý định của người dùng. Chỉ trả về một nhãn duy nhất.\n'
        'LEARN_RECOMMENDATION = hỏi hôm nay/hay tiếp theo học gì, nên học gì, hoặc chỉ chào khi người học đã quay lại.\n'
        'REVIEW_RECOMMENDATION = hỏi hôm nay ôn gì, muốn ôn tập, cần ôn nội dung nào.\n'
        'WRONG_ONLY_REVIEW = muốn làm lại/ôn lại những câu, bài hoặc phần đã làm sai/chưa đúng.\n'
        'SPECIFIC_LESSON_REVIEW = muốn ôn một bài cụ thể đã nêu tên.\n'
        'OTHER = mọi yêu cầu khác.\n'
        'Nhãn hợp lệ: LEARN_RECOMMENDATION, REVIEW_RECOMMENDATION, WRONG_ONLY_REVIEW, SPECIFIC_LESSON_REVIEW, OTHER.\n'
        f'Tin nhắn: {raw}'
    )
    try:
        started=time.perf_counter()
    ...
    # This fallback is only for LLM/network failure. It is not used for normal
    # routing when the classifier is available.
    q=_strip_vietnamese_diacritics(raw.casefold())
    if _is_pure_greeting(raw):
        return 'LEARN_RECOMMENDATION'
    if any(k in q for k in ('lam lai','on lai','phan sai','cau sai','chua dung')):
        return 'WRONG_ONLY_REVIEW'
    if re.search(r'\bon (tap|lai)\b', q) or 'muon on tap' in q or 'can on gi' in q:
        return 'REVIEW_RECOMMENDATION'
    if re.search(r'\bon (bai|tap)\b.+', q):
        return 'SPECIFIC_LESSON_REVIEW'
    if any(k in q for k in ('hoc gi','hoc tiep','nen hoc','hôm nay học','hom nay hoc','day gi')):
        return 'LEARN_RECOMMENDATION'
    return 'OTHER'


def _is_learning_intent_candidate(text: str):
    """Cheap gate for when to invoke the GenAI intent classifier."""
    q=_strip_vietnamese_diacritics(str(text or '').casefold()).strip()
    if not q:
        return False
    if _is_pure_greeting(text):
        return True
    discovery_terms=(
        'hoc gi','hoc tiep','nen hoc','hom nay hoc','day gi','co gi de hoc',
        'on tap','on gi','can on','hom nay on','on lai','review',
        'lam lai','cau sai','phan sai','chua dung','bao gio on'
    )
    return any(t in q for t in discovery_terms)
...
def _build_learning_discovery_blocks(user_id, course_id, course_name, intent='LEARN_RECOMMENDATION'):
    ...
    if intent=='LEARN_RECOMMENDATION':
        ...
        if today_plan_items:
            ...
            if plan_date:
                try:
                    plan_text=f' (dự kiến {plan_date.strftime("%d/%m/%Y") if hasattr(plan_date,"strftime") else str(plan_date)})'
                except Exception:
...
def proxy_chat(
    ...
    if (not data.free_chat_tutor and not data.action and selected_course_id is not None and data.text
        ...
        if not active_review_for_intent:
            ...
            # greetings are classified as LEARN_RECOMMENDATION.
            if not (_is_pure_greeting(data.text) and not profile.get('onboarding_completed') and not profile.get('learning_mode')):
                intent_result=_classify_learning_intent(data.text)
                print(f'[LEARNING INTENT ROUTER] user={user["id"]} intent={intent_result} text={data.text[:120]!r}')
    ...
    if intent_result in {'LEARN_RECOMMENDATION','REVIEW_RECOMMENDATION'}:
        ...
        return {'reply':msg,'model':'local-router','sources':[],'images':[],'content_blocks':blocks,'learning_progress':None,
...
{query_text}"""
    ...
    # Chat history/scope is inherited ONLY when GenAI says this is a follow-up
...
Trả lời ngắn gọn, đúng trọng tâm. Nếu context không đủ dữ kiện thì nói rõ điều đó. Nếu câu hiện tại ngắn hoặc thiếu chủ thể như "mẫu 4", "cái này", "ý là phần trên", "dịch hết", hãy hiểu nó là câu tiếp nối trực tiếp của context."""
    ...
    # Prompt history is strictly gated by the GenAI follow-up classifier:
    # - FOLLOW_UP  -> exactly the last 3 user/model exchanges (6 messages)
    # - NEW_TOPIC   -> no chat history at all
    # Runtime/RAG state may still be supplied separately when the current request
    # explicitly opens/continues a confirmed learning scope.
    prompt_history = recent_history[-6:] if chat_followup_detected and recent_history else []
...
RECENT CHAT — CHỈ DÙNG KHI FOLLOW_UP: {json.dumps(prompt_history, ensure_ascii=False, default=str, separators=(",", ":"))}
...
- FOLLOW_UP: dùng đúng 3 lượt user/model gần nhất làm ngữ cảnh cho câu trả lời.
- NEW_TOPIC: không dùng chat history của boxchat trước đó.
- Đây là lịch sử của chính boxchat hiện tại, không phải lịch sử học tập chung.
- Khi có history, dùng nó để hiểu "cậu", "đó", "bảng này", "câu trước", v.v.
...
def _is_review_schedule_request(text):
    ...
    phrases=(
        ...
        'hôm nay ôn tập gì','hom nay on tap gi','hôm nay ôn tập gì nhỉ','hom nay on tap gi nhi',
        'có gì cần ôn','co gi can on','cần ôn gì','can on gi','ôn lại','on lai','review hôm nay','review hom nay',
        'hôm nay học gì','hom nay hoc gi','hôm nay học gì nhỉ','hom nay hoc gi nhi'
    )
...
def _next_plan_lesson_for_welcome(user_id, course_id):
    """Return the next unfinished lesson from the user's active study plan."""
    if course_id is None:
        return None
    try:
        plans=_active_plans(user_id, course_id=int(course_id))
    except Exception as exc:
        print(f"[WELCOME PLAN] skipped: {type(exc).__name__}: {exc}")
        return None
    candidates=[]
    for plan in plans:
        for item in plan.get('items') or []:
            ...
            if not lesson:
                continue
            candidates.append((item.get('plan_date'), int(item.get('unit_index') or 0), lesson, item))
    if not candidates:
        return None
    candidates.sort(key=lambda x:(x[0] if x[0] is not None else date.max, x[1], x[2].casefold()))
...
def _review_selection_blocks(user_id, course_id, course_name, *, greeting_prefix=None):
    ...
    if wrong_count:
        ...
        try:
            ...
            print(f'[REVIEW SELECTION] wrong lesson labels skipped: {type(exc).__name__}: {exc}')
        finally:
            if conn:
                conn.close()
        if wrong_lessons:
...
def _build_review_reminder_blocks(user_id, course_id, course_name):
    """Return the unified daily learning/review briefing for proactive reminders."""
    msg,blocks,available=_build_learning_discovery_blocks(user_id,course_id,course_name,'LEARN_RECOMMENDATION')
    return (msg,blocks) if available else None
...
def _review_due_items(user_id, course_id):
    """Return wrong-answer review items whose scheduled date is today or earlier.

    PostgreSQL values are TIMESTAMPTZ in the durable review tables. We fetch the
    rows for the user/course and compare their date in Vietnam time in Python.
    This avoids relying on the database session timezone and guarantees that an
    item scheduled for any time today is considered due for the whole day.
    """
    today=_now_local().date()
    conn=db()
    try:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            ...
            grammar_all=[dict(x) for x in cur.fetchall()]
        def due(row):
            dt=row.get('next_review_at')
            if dt is None:
                return True
            try:
                local=_as_vn(dt)
                ...
                # Fail open for a malformed legacy timestamp: the item is already
                # in the durable retry queue, so it should not become invisible.
                return True
        vocab=[r for r in vocab_all if due(r)]
        grammar=[r for r in grammar_all if due(r)]
        print(f'[REVIEW DUE STATE] user={user_id} course_id={course_id} today={today.isoformat()} vocab_due={len(vocab)} grammar_due={len(grammar)} vocab_total={len(vocab_all)} grammar_total={len(grammar_all)}')
...
def _exercise_generate_deterministic_steps(lesson, question_pages, answer_pages):
    ...
    Only cropped exercise visual blocks are pre-attached to B1/B2. Original full-page
    source images stay in the Admin source inventory so the admin can optionally
    attach them manually, but they are NOT auto-selected for the step.
    """
    ...
    def _cropped_visuals(pages):
        ...
        for pg in pages or []:
            for im in pg.get('images') or []:
                ...
                kind=str(vision.get('kind') or im.get('kind') or '').strip().casefold()
                if kind != 'exercise_visual':
                    continue
                out.append({
                    'image_key':key,
                    'image_url':im.get('image_url'),
                    'page':pg.get('page'),
                    'caption':str(vision.get('description') or im.get('description') or vision.get('caption') or '').strip(),
                })
        return out

    q_images=_cropped_visuals(question_pages)
    a_images=_cropped_visuals(answer_pages)
    if not q_text:
...
def process_exercise_pdf_pages(pdf_source, reader, source_file: str, subject: str, lesson: str, question_pages=None, answer_pages=None):
    ...
    try:
        for page_no in selected:
            ...
            if png is None:
                png=render_pdf_page(pdf_source,page_no,dpi=220)
            ...
            visual_items=[]
            if gemini is not None and png:
                try:
                    visual_items=gemini_detect_exercise_visuals(
                        png, page_no, extracted_text=ocr_text, source_file=source_file
                    )
                except Exception as exc:
            ...
            for item in visual_items:
                ...
                visual_idx += 1
                safe_source=re.sub(r'[^A-Za-z0-9_.-]+','_',source_file)
                key=f'images/{safe_source}/exercise_page_{page_no:04d}/exercise_visual_{visual_idx:02d}.jpg'
                b2_put_bytes(key,image_bytes,'image/jpeg')
                kind=str(item.get('kind') or 'other').strip().lower()
                ...
                try:
                    with conn.cursor() as cur:
                        cur.execute("""INSERT INTO knowledge_images
                            ...
                            (source_file,subject,'Bài tập',lesson,None,page_no,key,b2_url(key),
                             f'Exercise visual [{kind}]: {description}',associated_text,bbox,width,height))
                    conn.commit()
                finally:
                    conn.close()
                stored.append({
                    'key':key,
                    'description':f'Exercise visual [{kind}]: {description}',
                    'associated_text':associated_text,
                    'bbox':bbox,
                    'page':page_no,
                    'kind':'exercise_visual',
                    'exercise_visual_kind':kind,
                    'image_scope':'chunk',
                    'unit_id':f'exercise:{tag}:page:{page_no}:visual:{visual_idx}',
                    'vision':{
                        'kind':'exercise_visual',
                        'exercise_visual_kind':kind,
            ...
            print(f'[EXERCISE VISUAL] page={page_no} scope={tag} detected={len(visual_items)} stored={visual_idx}')

            page_images[page_no]=stored
            units[0]['image_keys']=[
                x.get('key') for x in stored
                if str(x.get('kind') or '').strip().casefold()=='exercise_visual' and x.get('key')
            ]
            page_units[page_no]=units
            print(f'[EXERCISE OCR CACHE] page={page_no} scope={tag} text_chars={len(page_texts[page_no])} source_cached=1 cropped_visuals={len(units[0]["image_keys"])}')
    finally:
        if fitz_doc is not None:
            try: fitz_doc.close()
            except Exception: pass
